...
const fn generate_rtype(funct3: u32, funct7: u32, rtype: RType) -> u32 {
    let opcode = 51;
    let destination = (rtype.rd as u32) << 7;
    let funct3 = funct3 << 12;
    let first_source = (rtype.rs1 as u32) << 15;
    let second_source = (rtype.rs2 as u32) << 20;
...
fn generate_itype(funct3: u32, itype: IType) -> u32 {
    let opcode = 19;
    let destination = (itype.rd as u32) << 7;
    let funct3 = funct3 << 12;
    let source = (itype.rs1 as u32) << 15;
...
fn generate_itype_shifts(funct3: u32, funct7: u32, itype_shifts: ITypeShifts) -> u32 {
    let opcode = 19;
    let destination = (itype_shifts.rd as u32) << 7;
    let funct3 = funct3 << 12;
    let source = (itype_shifts.rs1 as u32) << 15;
...
fn generate_stype_memory(funct3: u32, stype_memory: STypeMemory) -> u32 {
    ...
    let funct3 = funct3 << 12;
    let base_address = (stype_memory.rs1 as u32) << 15;
    let source = (stype_memory.rd as u32) << 20;
...
fn generate_itype_memory(funct3: u32, itype_memory: ITypeMemory) -> u32 {
    let opcode = 3;
    let destination = (itype_memory.rd as u32) << 7;
    let funct3 = funct3 << 12;
    let base_address = (itype_memory.rs1 as u32) << 15;
...
fn generate_btype(funct3: u32, btype: BType) -> u32 {
    ...
    let funct3 = funct3 << 12;
    let first_source = (btype.rs1 as u32) << 15;
    let second_source = (btype.rs2 as u32) << 20;
...
const fn generate_jtype(jtype: JType) -> u32 {
    let opcode = 111;
    let destination = (jtype.rd as u32) << 7;
...
fn generate_itype_jump(funct3: u32, itypejump: ITypeJump) -> u32 {
    let opcode = 103;
    let destination = (itypejump.rd as u32) << 7;
    let funct3 = funct3 << 12;
    let source = (itypejump.rs1 as u32) << 15;
